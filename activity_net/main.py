    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(max_steps):
            ...
            if step % 50 == 0:
                ...
                print('Step %d: total loss = %.2f, regression loss = %.2f(%.3f sec)' % (
        ...
        saver.save(sess, os.path.join(ActivityNet.MODEL_DIR, 'turn_tap_model'))
...
def testing():
    ...
    # pickle.dump(result, open(os.path.join(ActivityNet.RES_DIR, 'turn_activity_net'), 'w'))
...
def eval():
    ...
    result = pickle.load(open(os.path.join(ActivityNet.RES_DIR, 'turn_activity_net'), 'r'))
    print(len(result),len(test))
    ret = {}
    for movie in test:
